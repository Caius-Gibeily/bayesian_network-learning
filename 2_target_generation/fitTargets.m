function [L3,L4,rois] = fitTargets(coordsTD,coordsASD,gs,radius,alpha,alignWindow)
...
for k = 2:length(coordsTD{1})-2
    ...
    nns = zeros(length(xy),1);
    tab = squareform(pdist(xy,"euclidean"));
    ...
    vqsmoth = imgaussfilt(vq,1.5);

    vqsmoth = zscore(vqsmoth,0,"all");
    rois{k} =vqsmoth;
    
    if k > 3
        vqsmoth = alpha * rois{k} + (1-alpha) * rois{k-1} + (1-alpha)^2 * rois{k-2} + (1-alpha)^3 * rois{k-3};
    end
    
    L3{k} = findgauspeaks(x,y,vqsmoth,20,max(maxes),unique(L3{k-1}));
    ...
    % shg
    % clf
    % display(max(L3{k},[],"all"))
    display(k)
end

%% Post-processing
% Short-range alignment
window = alignWindow; %20
wsize = 1:window;
L3a = L3;
